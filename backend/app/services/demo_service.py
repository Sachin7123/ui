...
class StreamHub:
    ...
    def unsubscribe(self, channel: str, channel_queue: queue.Queue[EventPayload]) -> None:
        with self._lock:
    ...
    def queue_depth(self) -> int:
        with self._lock:
            return sum(listener.qsize() for listeners in self._listeners.values() for listener in listeners)


class DemoService:
    def __init__(self, storage: StorageBackend | None = None, artifact_repo: ArtifactRepository | None = None) -> None:
        self._storage = storage or build_storage_backend()
        self._artifact_repo = artifact_repo or ArtifactRepository()
        self._data_source_mode = os.getenv("DATA_SOURCE_MODE", "simulator").strip().lower()
        self._hub = StreamHub()
    ...
    def start(self) -> None:
        ...
        if self._data_source_mode in {"openenv", "artifacts", "artifact"}:
            ...
            self._thread = threading.Thread(target=self._artifact_stream_loop, daemon=True)
            self._thread.start()
            return
    ...
    def pipeline_overview(self) -> PipelineOverviewResponse:
        ...
        return PipelineOverviewResponse(
            ...
            reward_series=self._build_metric_series("reward", "Reward", ["#38bdf8", "#8b5cf6", "#14b8a6"]),
            loss_series=self._build_metric_series("loss", "Loss", ["#fb7185", "#f59e0b", "#e879f9"]),
            throughput_series=self._build_metric_series("throughput", "Tokens/sec", ["#22c55e", "#2dd4bf", "#60a5fa"]),
    ...
    def command_center(self) -> CommandCenterResponse:
        ...
        return CommandCenterResponse(
            ...
            active_runs=runs,
            reward_series=self._build_metric_series("reward", "Reward", ["#38bdf8", "#8b5cf6", "#14b8a6"], limit=90),
            loss_series=self._build_metric_series("loss", "Loss", ["#fb7185", "#f59e0b", "#e879f9"], limit=90),
            throughput_series=self._build_metric_series("throughput", "Tokens/sec", ["#22c55e", "#2dd4bf", "#60a5fa"], limit=90),
            gpu_series=self._build_metric_series("gpu_utilization", "GPU %", ["#f59e0b", "#38bdf8", "#8b5cf6"], limit=90),
    ...
    def historical_analytics(self) -> HistoricalAnalyticsResponse:
        ...
        best_runs = sorted(runs, key=lambda item: (item.success_rate, item.reward_latest), reverse=True)[:4]
        return HistoricalAnalyticsResponse(
            ...
            reward_trends=self._build_metric_series("reward", "Reward", ["#38bdf8", "#8b5cf6", "#14b8a6"], limit=180),
            loss_trends=self._build_metric_series("loss", "Loss", ["#fb7185", "#f59e0b", "#e879f9"], limit=180),
            throughput_trends=self._build_metric_series("throughput", "Tokens/sec", ["#22c55e", "#2dd4bf", "#60a5fa"], limit=180),
            best_runs=best_runs,
    ...
    def inspector(self, *, limit: int = 12) -> InspectorResponse:
        ...
        rewards_by_step = {(reward.run_id, reward.step): reward for reward in self._storage.recent_rewards(limit=80)}
        ...
        return InspectorResponse(
            stats=[
                ...
                MetricCard(
                    ...
                    value=str(sum(1 for example in examples if example.output.label == 'bad_generation')),
                    delta="Anomaly-aware",
                    direction="down",
            ...
            examples=examples,
            prompt_series=self._build_metric_series("reward", "Reward", ["#38bdf8", "#8b5cf6", "#14b8a6"], limit=80),
        )

    def alerts_center(self, *, limit: int = 24) -> AlertsResponse:
        alerts = self._storage.recent_alerts(limit=limit)
        events = [event for event in self._storage.recent_events(limit=limit * 2) if event.severity in {"high", "critical"}]
        return AlertsResponse(
            stats=[
                ...
                MetricCard(
                    ...
                    value=str(sum(1 for event in events if event.severity == 'critical')),
                    delta="Auto-detected",
                    direction="down",
                ...
                MetricCard(
                    ...
                    value=str(sum(1 for alert in alerts if alert.alert_type == 'repair_regression')),
    ...
    def remorph_engine(self, *, limit: int = 12) -> RemorphEngineResponse:
        ...
        return RemorphEngineResponse(
            ...
            repairs=repairs,
            recent_events=[event for event in self._storage.recent_events(limit=limit * 2) if event.event_type.startswith("repair")][:limit],
        )
    ...
    def system_health(self) -> SystemHealthResponse:
        ...
        return SystemHealthResponse(
            ...
            storage_latency_ms=round(sum(self._storage_latency_ms) / max(1, len(self._storage_latency_ms)), 2),
            active_run_count=len([run for run in runs if run.status in {"running", "degraded", "completed"}]),
    ...
    def _bootstrap_from_openenv_artifacts(self) -> None:
        ...
        selected = sorted(grouped.items(), key=lambda item: len(item[1]), reverse=True)[:6]
        for index, (scenario_id, rows) in enumerate(selected):
            ...
            success_rate = sum(1 for row in rows if bool(row.get("success"))) / max(1, len(rows))
            avg_raw_reward = sum(float(row.get("raw_reward", 0.0)) for row in rows) / max(1, len(rows))
            started = (now - timedelta(minutes=45 - index * 6)).replace(microsecond=0).isoformat()
            updated = (now - timedelta(seconds=index * 4)).replace(microsecond=0).isoformat()
            run = RunRecord(
                run_id=run_id,
                name=scenario_id.replace("-", " "),
                model_name=str(training_summary.get("trainer", "supervised_structured_policy")),
                ...
                last_alert=None if rows[-1].get("success") else "openenv replay mismatch",
                tags=["openenv", "artifact", str(rows[-1].get("policy_name", "policy"))],
            )
            self._timed_store(lambda run_record=run: self._storage.upsert_run(run_record))
            self._record_ingest()

            for offset, row in enumerate(rows):
                ts = (now - timedelta(seconds=(len(rows) - offset) * 3 + index * 4)).replace(microsecond=0).isoformat()
                raw_reward = float(row.get("raw_reward", 0.0))
                confidence = float(row.get("confidence", 0.0))
                metric_seed = f"{scenario_id}:{offset}"
                metrics = [
                    MetricSample(
                        metric_id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"{metric_seed}:reward")),
                        run_id=run_id,
                        metric_name="reward",
                        split="eval",
                        step=offset + 1,
                    ...
                    MetricSample(
                        metric_id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"{metric_seed}:loss")),
                        run_id=run_id,
                        metric_name="loss",
                        split="eval",
                        step=offset + 1,
                    ...
                    MetricSample(
                        metric_id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"{metric_seed}:throughput")),
                        run_id=run_id,
                    ...
                    MetricSample(
                        metric_id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"{metric_seed}:gpu")),
                        run_id=run_id,
                ...
                event = EventRecord(
                    event_id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"{metric_seed}:event")),
                    run_id=run_id,
                    event_type=f"openenv.{row.get('belief', 'step')}",
                    severity=self._severity_from_reward(raw_reward, bool(row.get("success"))),
                    timestamp=ts,
                ...
                prompt = PromptRecord(
                    prompt_id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"{metric_seed}:prompt")),
                    run_id=run_id,
                    step=offset + 1,
                    timestamp=ts,
                    scenario=scenario_id,
                    input_text=str(observation.get("error_signal", {}).get("message") or "Analyze failed API request and propose safe repair."),
                    metadata={"observation": observation},
                )
                self._store_prompt(prompt)

                output = OutputRecord(
                    output_id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"{metric_seed}:output")),
                    run_id=run_id,
                    prompt_id=prompt.prompt_id,
                    step=offset + 1,
                    timestamp=ts,
                    output_text=json.dumps(action),
                    label="good_generation" if bool(row.get("success")) else "bad_generation",
                    score=confidence,
                    metadata={"belief": row.get("belief"), "telemetry_group": row.get("telemetry_group")},
                )
                self._store_output(output)

                reward_record = RewardRecord(
                    reward_id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"{metric_seed}:reward_record")),
                    run_id=run_id,
                    step=offset + 1,
                    timestamp=ts,
                    reward_total=raw_reward,
                    components={key: float(value) for key, value in (row.get("reward_breakdown") or {}).items() if isinstance(value, (int, float))},
                )
                self._store_reward(reward_record)

                if bool(row.get("done")) and not bool(row.get("success")):
                    alert = AlertRecord(
                        alert_id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"{metric_seed}:alert")),
                        run_id=run_id,
                ...
                if action.get("action_type") in {"repair_route", "repair_payload", "repair_auth"}:
                    failed_request = observation.get("failed_request", {}) if isinstance(observation, dict) else {}
                    healed_request = {
                    ...
                    repair = RepairRecord(
                        repair_id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"{metric_seed}:repair")),
                        run_id=run_id,
                        scenario_id=scenario_id,
                        timestamp=ts,
                        failed_request=failed_request if isinstance(failed_request, dict) else {},
                        repair_reasoning=str(action.get("reason") or "Derived from OpenEnv artifact replay."),
                        healed_request=healed_request,
                        retry_result="200 OK" if bool(row.get("success")) else "needs_review",
        ...
        self._append_openenv_history_metrics(reward_history=reward_history, loss_history=loss_history, now=now)

    def _append_openenv_history_metrics(self, *, reward_history: list[dict], loss_history: list[dict], now: datetime) -> None:
        for index, row in enumerate(reward_history[:40]):
            ...
            ts = (now - timedelta(minutes=40 - index)).replace(microsecond=0).isoformat()
            value = float(row.get("average_raw_reward", 0.0))
            self._store_metric(
                MetricSample(
                    metric_id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"openenv-history-reward:{split}:{epoch}")),
                    run_id=run_id,
                    metric_name="reward",
                    split=split,
                    step=epoch + 1,
        ...
        for index, row in enumerate(loss_history[:40]):
            ...
            run_id = f"openenv-summary-{split}"
            ts = (now - timedelta(minutes=35 - index)).replace(microsecond=0).isoformat()
            mismatch_rate = float(row.get("mismatch_rate", 0.0))
            self._store_metric(
                MetricSample(
                    metric_id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"openenv-history-loss:{split}:{epoch}")),
                    run_id=run_id,
                    metric_name="loss",
                    split=split,
                    step=epoch + 1,
    ...
    def _artifact_stream_loop(self) -> None:
        while not self._stop_event.is_set():
            ...
            if recent_events:
                ...
                self._publish("logs", EventPayload(channel="logs", event_type=event.event_type, timestamp=event.timestamp, data=event.model_dump(mode="json")))
            ...
            if recent_alerts and self._artifact_cursor % 3 == 0:
                ...
                self._publish("alerts", EventPayload(channel="alerts", event_type=alert.alert_type, timestamp=alert.timestamp, data=alert.model_dump(mode="json")))
            ...
            if recent_repairs and self._artifact_cursor % 2 == 0:
                ...
                self._publish("repairs", EventPayload(channel="repairs", event_type="repair.update", timestamp=repair.timestamp, data=repair.model_dump(mode="json")))
    ...
    def _new_run(self, slot: int) -> SimRun:
        ...
        return SimRun(
            ...
            tags=["remorph", "api-healing", "reward-model"] if slot % 2 == 0 else ["observability", "realtime"],
        )
    ...
    def _tick_run(self, run: SimRun) -> None:
        if run.current_step >= run.total_steps:
            completion_event = self._build_event(run, "run.completed", "info", f"{run.name} reached the final checkpoint.")
        ...
        if self._rng.random() < 0.018:
            ...
            alert = self._build_alert(run, "reward_drop", "high", "Reward collapse detected on live stream.")
        elif self._rng.random() < 0.018:
            loss_penalty = self._rng.uniform(0.4, 0.9)
            alert = self._build_alert(run, "loss_spike", "critical", "Loss spike exceeded expected band.")
        elif self._rng.random() < 0.012:
            alert = self._build_alert(run, "stalled_run", "medium", "Step progression slowed below expected throughput.")
        elif self._rng.random() < 0.010:
            alert = self._build_alert(run, "bad_generation", "medium", "Generated output drifted from expected repair pattern.")

        run.reward_latest = max(0.05, 0.25 + progress * 0.9 + math.sin(run.current_step / 11 + run.slot) * 0.06 - anomaly_penalty)
        run.loss_latest = max(0.05, 1.9 - progress * 1.45 + abs(math.sin(run.current_step / 9 + run.slot)) * 0.08 + loss_penalty)
        run.success_rate = max(0.12, min(0.99, 0.42 + progress * 0.48 - anomaly_penalty * 0.35))
        run.throughput = max(420.0, 950 + math.sin(run.current_step / 8 + run.slot) * 160 + self._rng.uniform(-70, 60))
        run.gpu_util = max(28.0, min(99.0, 72 + math.cos(run.current_step / 10 + run.slot) * 14 + self._rng.uniform(-5, 5)))
        run.anomaly_score = min(1.0, 0.12 + abs(anomaly_penalty) * 2.6 + abs(loss_penalty) * 0.5)
        ...
        timestamp = iso_now()
        metrics = [
            MetricSample(metric_id=str(uuid.uuid4()), run_id=run.run_id, metric_name="reward", split="train", step=run.current_step, timestamp=timestamp, value=run.reward_latest),
            MetricSample(metric_id=str(uuid.uuid4()), run_id=run.run_id, metric_name="loss", split="train", step=run.current_step, timestamp=timestamp, value=run.loss_latest),
            MetricSample(metric_id=str(uuid.uuid4()), run_id=run.run_id, metric_name="throughput", split="train", step=run.current_step, timestamp=timestamp, value=run.throughput),
            MetricSample(metric_id=str(uuid.uuid4()), run_id=run.run_id, metric_name="gpu_utilization", split="train", step=run.current_step, timestamp=timestamp, value=run.gpu_util),
        ]
        for metric in metrics:
            self._store_metric(metric)
        ...
        if run.current_step % 6 == 0:
            ...
            output = self._build_output(run, prompt.prompt_id, alert_type=alert.alert_type if alert else None)
            self._store_prompt(prompt)
            self._store_output(output)

        if run.current_step % 10 == 0:
            repair = self._build_repair(run, alert_type=alert.alert_type if alert else None)
            self._store_repair(repair)
        ...
        self._publish("runs", EventPayload(channel="runs", event_type="run.update", timestamp=iso_now(), data=run_record.model_dump(mode="json")))
    ...
    def _store_event(self, event: EventRecord) -> None:
        ...
        self._publish(
            "logs",
            EventPayload(channel="logs", event_type=event.event_type, timestamp=event.timestamp, data=event.model_dump(mode="json")),
        )
    ...
    def _store_alert(self, alert: AlertRecord) -> None:
        ...
        self._publish(
            "alerts",
            EventPayload(channel="alerts", event_type=alert.alert_type, timestamp=alert.timestamp, data=alert.model_dump(mode="json")),
        )
    ...
    def _store_repair(self, repair: RepairRecord) -> None:
        ...
        self._publish(
            "repairs",
            EventPayload(channel="repairs", event_type="repair.update", timestamp=repair.timestamp, data=repair.model_dump(mode="json")),
        )
    ...
    def _build_event(self, run: SimRun, event_type: str, severity: str, message: str, payload: dict | None = None) -> EventRecord:
        return EventRecord(
    ...
    def _build_output(self, run: SimRun, prompt_id: str, *, alert_type: str | None) -> OutputRecord:
        is_bad = alert_type in {"bad_generation", "mode_collapse"} or self._rng.random() < 0.08
        return OutputRecord(
    ...
    def _build_alert(self, run: SimRun, alert_type: str, severity: str, detail: str) -> AlertRecord:
        return AlertRecord(
    ...
    def _build_repair(self, run: SimRun, *, alert_type: str | None) -> RepairRecord:
        ...
        outcome = "failed" if alert_type == "repair_regression" else outcomes[self._rng.randint(0, len(outcomes) - 1)]
        failed_request = {
            "method": "POST",
            "path": "/api/v1/orders",
        ...
        if outcome != "abstained":
            healed_request = {
                "method": "POST",
                "path": "/api/v2/orders",
                "headers": {"Authorization": "Bearer demo-token", "x-tenant-id": "north"},
                "body": {"order_id": f"O-{run.current_step}", "amount": 1900},
            }
        return RepairRecord(
            ...
            healed_request=healed_request,
            retry_result="200 OK" if outcome == "success" else "safe_abstain" if outcome == "abstained" else "422 persists",
    ...
    def _overview_cards(self, runs: list[RunRecord]) -> list[MetricCard]:
        ...
        return [
            MetricCard(
                id="active-runs",
                label="Active Runs",
                value=str(len([run for run in runs if run.status in {'running', 'degraded'}])),
    ...
    def _build_metric_series(self, metric_name: str, label: str, colors: list[str], *, limit: int = 120) -> list[Series]:
        runs = self._current_runs(limit=3)
        series: list[Series] = []
        for index, run in enumerate(runs):
            samples = self._storage.recent_metrics([metric_name], limit_per_metric=limit, run_id=run.run_id)
            points = [TimePoint(timestamp=sample.timestamp, value=sample.value, run_id=run.run_id, label=run.name) for sample in samples]
            series.append(
                Series(
    ...
    def _failure_causes(self, alerts: list[AlertRecord]) -> list[FailureCauseRow]:
        ...
        return [FailureCauseRow(name=name.replace("_", " ").title(), count=count, severity=severity_rank[name]) for name, count in ordered]

    def _repair_success_rate(self, repairs: list[RepairRecord]) -> float:
        if not repairs:
            return 0.0
        return sum(1 for repair in repairs if repair.outcome == "success") / len(repairs)
    ...
    def _paginate(self, items: list, *, page: int, page_size: int) -> PageResponse:
        ...
        return PageResponse(items=items[start:end], total=len(items), page=safe_page, page_size=safe_size)
