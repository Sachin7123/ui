...
from app.repositories.base import database_path, event_log_path, runtime_root
...
class ObservabilityStorage:
        ...
        with self._lock:
            self._conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS runs (
                    ...
                    reward_latest REAL NOT NULL,
                    loss_latest REAL NOT NULL,
    ...
    def _insert_simple(self, table: str, values: tuple[Any, ...]) -> None:
        ...
        with self._lock:
            self._conn.execute(f"INSERT OR REPLACE INTO {table} VALUES ({placeholders})", values)
            self._conn.commit()
    ...
    def recent_metrics(self, metric_names: list[str], *, limit_per_metric: int = 180, run_id: str | None = None) -> list[MetricSample]:
        ...
        with self._lock:
            for metric_name in metric_names:
                ...
                samples.extend(self._metric_from_row(row) for row in rows)
    ...
    def recent_alerts(self, *, limit: int = 50) -> list[AlertRecord]:
        with self._lock:
            rows = self._conn.execute("SELECT * FROM alerts ORDER BY timestamp DESC LIMIT ?", (limit,)).fetchall()
        return [self._alert_from_row(row) for row in rows]

    def recent_repairs(self, *, limit: int = 40) -> list[RepairRecord]:
        with self._lock:
            rows = self._conn.execute("SELECT * FROM repairs ORDER BY timestamp DESC LIMIT ?", (limit,)).fetchall()
            ...
            result.append(
                {
                    ...
                    "label": row["label"],
                    "score": row["score"],
                    "output_metadata": json.loads(row["output_metadata_json"]) if row["output_id"] else {},
                }
            )
        return result
    ...
    def recent_rewards(self, *, limit: int = 40) -> list[RewardRecord]:
        with self._lock:
            rows = self._conn.execute("SELECT * FROM rewards ORDER BY timestamp DESC LIMIT ?", (limit,)).fetchall()
        return [
            RewardRecord(
    ...
    def count_open_alerts(self) -> int:
        with self._lock:
            row = self._conn.execute("SELECT COUNT(*) AS count FROM alerts WHERE resolved = 0").fetchone()
        return int(row["count"]) if row else 0
    ...
    def _metric_from_row(self, row: sqlite3.Row) -> MetricSample:
        return MetricSample(
    ...
    def _repair_from_row(self, row: sqlite3.Row) -> RepairRecord:
        return RepairRecord(
